...
loadSamplesFile <- function(file, reads_folder, column){
    ## debug
    file = opt$samplesFile; reads_folder = opt$rawFolder; column = opt$samplesColumn
    ##
    if (!file.exists(file)) {
...
samples <- loadSamplesFile(opt$samplesFile, opt$rawFolder, opt$samplesColumn)
procs <- prepareCore(opt$procs)
umiQuery <- sampleList(samples, opt$rawFolder, opt$samplesColumn)
